...
import fs from "fs";
import { notoSansRegular, notoSansBold } from "./src/lib/notoSansFont.ts";

// We can't import .ts directly in node easily; instead read the font file again from disk via base64.
...
function build(lang) {
  const copies = {
    cs: {
      ...
      headerParam: "PARAMETR", headerLong: "DLOUHODOBÝ", headerUs: "DAU AN",
    },
    vi: {
      ...
      headerParam: "TIÊU CHÍ", headerLong: "DÀI HẠN", headerUs: "DAU AN",
    },
  };
  const c = copies[lang];
  const pdf = new jsPDF({ unit: "mm", format: "a4", orientation: "portrait" });
  pdf.addFileToVFS("NotoSans-Regular.ttf", reg);
  ...
  pdf.setFillColor(26,26,26); pdf.setGState(pdf.GState({opacity:0.55}));
  pdf.rect(0,0,W,95,"F"); pdf.setGState(pdf.GState({opacity:1}));

  pdf.setTextColor(255,255,255); pdf.setFont(F,"bold"); pdf.setFontSize(11);
  pdf.text("DAU AN",15,15);
  pdf.setTextColor(...gold); pdf.text("s.r.o.",30,15);
  pdf.setTextColor(...gold); pdf.setFontSize(8); pdf.text(c.eyebrow,15,45);
  pdf.setTextColor(255,255,255); pdf.setFontSize(24); pdf.text(c.title1,15,58);
  ...
  pdf.setTextColor(...gold); pdf.setFont(F,"bold"); pdf.setFontSize(8);
  ...
  pdf.setDrawColor(...gold); pdf.setLineWidth(0.6); pdf.line(15,y+2,45,y+2);
  y+=9;
  pdf.setFillColor(...charcoal); pdf.rect(15,y,W-30,7,"F");
  pdf.setTextColor(255,255,255); pdf.setFont(F,"bold"); pdf.setFontSize(8);
  pdf.text(c.headerParam,18,y+4.7);
  pdf.text(c.headerLong,78,y+4.7);
  pdf.setTextColor(...gold); pdf.text(c.headerUs,138,y+4.7);
  y+=7;
  pdf.setFont(F,"normal"); pdf.setFontSize(8.5);
  c.whyRows.forEach((row,i)=>{
    pdf.setFillColor(i%2===0?255:243, i%2===0?255:240, i%2===0?255:233);
    ...
    pdf.setTextColor(...charcoal); pdf.text(pdf.splitTextToSize(row[2],55),138,y+5.8);
    y+=9;
  });
  ...
  pdf.setFont(F,"normal"); pdf.setFontSize(9);
  pdf.text(c.ctaText,15,y+18);
  pdf.setFont(F,"bold"); pdf.setFontSize(11);
...
const cs = build("cs");
fs.writeFileSync("/tmp/pdfqa/cs.pdf", Buffer.from(cs.output("arraybuffer")));
const vi = build("vi");
fs.writeFileSync("/tmp/pdfqa/vi.pdf", Buffer.from(vi.output("arraybuffer")));
console.log("ok");
