-- A model version that has produced at least one lead estimate can no longer change its params; create a new version instead.
...
begin
  if new.params is distinct from old.params
    ...
    raise exception 'calculator_params % is frozen: it has produced lead estimates. Insert a new model_version instead.', old.model_version;
  end if;
  return new;
end $$;
