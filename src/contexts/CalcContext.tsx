...
/**
 * Sdílený stav kalkulačky (lokalita, dispozice, hosté, plocha, sezóna, vybavení).
...
export const CalcProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [guests, setGuests] = useState<number>(initial?.guests ?? SIZE_PRESET[size0].guests);
  ...
  const value = useMemo<CalcState>(() => ({
    location, setLocation, size, pickSize, guests, setGuests, m2, setM2,
    season, setSeason, furn, setFurn, fromShare: initial !== null,
  }), [location, size, guests, m2, season, furn, initial]);
