 ...
 * Křivka `str` = s Antam Homes (hlavní číslo kalkulačky), `strMarket` = totéž
 * při průměru trhu; obě z téhož ownerMonthly, stejné energie, obnova i start.
 */
export const fiveYear = (location: CalcLoc, size: SizeKey, m2: number, furn: Furn) => {
  ...
  const net = year.antam.net;
  const netMarket = year.market.net;
  ...
  const lt = [0], str = [-setup], strMarket = [-setup];
  ...
  return { lt, str, strMarket, rent, y2, netMarket, setup, kit, projectFee, energy, renew, payback, cross, gap: str[HORIZON_MONTHS] - lt[HORIZON_MONTHS] };
};
